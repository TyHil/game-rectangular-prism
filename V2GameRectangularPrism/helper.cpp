...
void grid(uint8_t XDim, uint8_t YDim, Adafruit_SSD1306 & display) { //draw grid for Clonium or Minesweeper
  const bool area = XDim > 8 or YDim > 4;
  for (uint8_t i = 0; i <= XDim * (-8 * area + 16); i += 16 / (area + 1))
    display.drawLine(i, 0, i, YDim * (-8 * area + 16), WHITE);
  for (uint8_t i = 0; i <= YDim * (-8 * area + 16); i += 16 / (area + 1))
    display.drawLine(0, i, XDim * (-8 * area + 16), i, WHITE);
