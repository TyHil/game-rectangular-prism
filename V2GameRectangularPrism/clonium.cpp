...
void displayCloniumSetup(Adafruit_SSD1306 & display, int16_t screen, uint8_t XDim, uint8_t YDim) {
  ...
  for (uint8_t i = 0; i < 4; i++) {
    ...
    if (i == 0 or i == 1) display.setTextSize(2);
    if (i == 0) display.print(XDim);
    else if (i == 1) display.print(YDim);
    else display.print(names[i + 1 + (i == 3)]);
    if (i == 3) {
  ...
  uint8_t y = 16;
  for (uint8_t j = 0; j < min(screen - (screen == 3), 3); j++)
    y += (names[j].length() + 1) * 6 + (j == 0) * 18 + (j == 1) * 6;
...
CloniumBoard::CloniumBoard(uint8_t setXDim, uint8_t setYDim) {
  ...
  for (uint8_t x = 0; x < setXDim; x++)
    for (uint8_t y = 0; y < setYDim; y++) data[x][y] = 0;
    ...
    for (uint8_t y = 0; y < YDim; y++) {
      if (getDots(x, y) > 0) {
        display.fillRect(
          x * sixteenOverArea + 1,
          y * sixteenOverArea + 1,
          (16 / (area + 1)) - 1 - (x == 8 * (area + 1) - 1),
          (16 / (area + 1)) - 1 - (y == 4 * (area + 1) - 1),
          !(getTeam(x, y) - 1)
        ); //fill with reverse team color
        display.fillRect(
          x * sixteenOverArea + (6 / (area + 1) + area),
          y * sixteenOverArea + (3 / (area + 1) + area),
          (4 / (area + 1)),
          (4 / (area + 1)),
          getTeam(x, y) - 1
        ); //1st dot
      ...
      if (getDots(x, y) > 1)
        display.fillRect(
          x * sixteenOverArea + (9 / (area + 1) + area),
          y * sixteenOverArea + (9 / (area + 1) + area),
          (4 / (area + 1)),
          (4 / (area + 1)),
          getTeam(x, y) - 1
        ); //2nd dot
      if (getDots(x, y) > 2)
        display.fillRect(
          x * sixteenOverArea + (3 / (area + 1) + area),
          y * sixteenOverArea + (9 / (area + 1) + area),
          (4 / (area + 1)),
          (4 / (area + 1)),
          getTeam(x, y) - 1
...
uint8_t CloniumBoard::numNextToNumBonus(uint8_t x, uint8_t y, uint8_t nextTo, int8_t bonus, uint8_t turn, uint8_t lastDir) {
  uint8_t c = 0;
  if (y != YDim - 1 and getDots(x, y + 1) == nextTo and lastDir != 1 and getTeam(x, y + 1) - 1 != turn)
    c += bonus; //bonus for putting a 2 next to an enemy's 1
  if (x != XDim - 1 and getDots(x + 1, y) == nextTo and lastDir != 3 and getTeam(x + 1, y) - 1 != turn)
    c += bonus;
  if (y != 0 and getDots(x, y - 1) == nextTo and lastDir != 2 and getTeam(x, y - 1) - 1 != turn)
    c += bonus;
  if (x != 0 and getDots(x - 1, y) == nextTo and lastDir != 4 and getTeam(x - 1, y) - 1 != turn)
    c += bonus;
  return c;
}
...
uint8_t CloniumBoard::recursiveEval(uint8_t x, uint8_t y, uint8_t lastDir, uint8_t turn) {
  if (loopPrevention[x][y]) return 0; //stop a looping situation like a square of 3s
    ...
    if (getDots(x, y) == 1) {
      ...
      c += 3; //3 value
      if (y != YDim - 1 and getDots(x, y + 1) == 3 and lastDir != 1)
        c += recursiveEval(x, y + 1, 2, turn); //call the same function on neighboring places as a 4 expands into them
      if (x != XDim - 1 and getDots(x + 1, y) == 3 and lastDir != 3)
        c += recursiveEval(x + 1, y, 4, turn);
...
void CloniumBoard::CPUMove(uint8_t turn, Adafruit_SSD1306 & display) {
  ...
  for (uint8_t i = 0; i < XDim; i++)
    for (uint8_t j = 0; j < YDim; j++) f[i][j] = 0;
...
void CloniumBoard::mover(uint8_t x, uint8_t y, uint8_t turn) {
  if (getDots(x, y) < 3) { //add 1 to places
    ...
    if (x != 0)
      mover(x - 1, y, turn); //call the same function on neighboring places as a 4+ expands into them
...
void Clonium::winCheck(Adafruit_SSD1306 & display) {
  ...
  for (uint8_t x = 0; x < board.XDim; x++) { //check for win by counting all of the enemies pieces
    for (uint8_t y = 0; y < board.YDim; y++)
      if (board.getTeam(x, y) - 1 == !turn) count = 0;
  }
